...
library UniversalRouterHelper {
    ...
    function pairAndReservesFor(address factory, address tokenA, address tokenB)
        private
        view
        returns (address pair, uint256 reserveA, uint256 reserveB, uint swapFee)
    {
        address token0;
        (pair, token0) = pairAndToken0For(factory, tokenA, tokenB);
        (uint256 reserve0, uint256 reserve1, uint fee) = IZFPair(pair).getReservesAndParameters();
    ...
    function getAmountIn(uint256 amountOut, uint256 reserveIn, uint256 reserveOut, uint swapFee)
        internal
        pure
    ...
    function getAmountInMultihop(address factory, uint256 amountOut, address[] calldata path)
        internal
        view
        returns (uint256 amount, address pair)
    {
        ...
        for (uint256 i = path.length - 1; i > 0; i--) {
            ...
            uint256 reserveOut;
            uint swapFee;
            (pair, reserveIn, reserveOut, swapFee) = pairAndReservesFor(factory, path[i - 1], path[i]);
